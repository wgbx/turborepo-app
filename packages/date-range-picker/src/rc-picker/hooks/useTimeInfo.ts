...
export default function useTimeInfo<DateType extends object = any>(
  ...
  const [mergedDisabledHours, mergedDisabledMinutes, mergedDisabledSeconds, mergedDisabledMilliseconds] = React.useMemo(
  ...
  const getAllUnits = React.useCallback(
    (
      ...
      // Minutes
      const getMinuteUnits = (nextHour: number) => generateUnits(0, 59, minuteStep, hideDisabledOptions, getDisabledMinutes(nextHour))
  ...
  const getValidTime = (nextTime: DateType, certainDate?: DateType) => {
    ...
    if (certainDate) {
      ...
      const [targetRowHourUnits, targetGetMinuteUnits, targetGetSecondUnits, targetGetMillisecondUnits] = getAllUnits(
        targetDisabledHours,
