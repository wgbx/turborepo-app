...
export default function TimePanelBody<DateType extends object = any>(props: SharedTimeProps<DateType>) {
  ...
  const getUnitValue = (func: 'getHour' | 'getMinute' | 'getSecond' | 'getMillisecond'): [valueNum: number, pickerNum: number] => {
  ...
  const hourUnits = React.useMemo(() => {
    ...
    return isAM(hour) ? rowHourUnits.filter((h) => isAM(h.value as number)) : rowHourUnits.filter((h) => !isAM(h.value as number))
