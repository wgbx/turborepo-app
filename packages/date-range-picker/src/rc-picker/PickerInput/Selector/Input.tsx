...
const Input = React.forwardRef<InputRef, InputProps>((props, ref) => {
  ...
  const { prefixCls, input: Component = 'input', classNames, styles } = React.useContext(PickerContext)
  ...
  const [focusCellIndex, setFocusCellIndex] = React.useState<number | null>(null)
  const [forceSelectionSyncMark, forceSelectionSync] = React.useState<object | null>(null)
  ...
  const onFormatKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (event) => {
    ...
    const offsetCellValue = (offset: number) => {
      const result = getMaskRange(cellFormat || '')
      const [rangeStart, rangeEnd, rangeDefault] = result || [0, 0, 0]
    ...
    if (nextFillText !== null) {
      ...
      triggerInputChange(nextFocusValue.slice(0, format?.length || 0))
    }
