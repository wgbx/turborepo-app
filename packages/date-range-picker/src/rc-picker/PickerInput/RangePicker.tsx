...
function RangePicker<DateType extends object = any>(props: RangePickerProps<DateType>, ref: React.Ref<RangePickerRef>) {
  // ========================= Prop =========================
  ...
  const mergedShowTime = React.useMemo<PopupShowTimeConfig<DateType> & Pick<RangeTimeProps<DateType>, 'defaultOpenValue'>>(() => {
    if (!showTime) {
      return null
    }
  ...
  const onSelectorClick: React.MouseEventHandler<HTMLDivElement> = (event) => {
    ...
    if (!selectorRef.current.nativeElement.contains((rootNode as Document | ShadowRoot).activeElement ?? document.activeElement)) {
  ...
  const [activeInfo, setActiveInfo] = React.useState<[activeInputLeft: number, activeInputRight: number, selectorWidth: number]>([0, 0, 0])
