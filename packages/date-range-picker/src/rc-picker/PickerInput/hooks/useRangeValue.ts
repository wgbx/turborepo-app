...
function useUtil<MergedValueType extends object[], DateType extends MergedValueType[number] = any>(
  ...
  const getDateTexts = (dates: MergedValueType) => {
    return dates.map((date) => formatValue(date, { generateConfig, locale, format: formatList[0] })) as any as ReplaceListType<
      Required<MergedValueType>,
      string
    >
  }
...
function orderDates<DateType extends object, DatesType extends DateType[]>(dates: DatesType, generateConfig: GenerateConfig<DateType>) {
...
export function useInnerValue<ValueType extends DateType[], DateType extends object = any>(
  ...
  const triggerCalendarChange: TriggerCalendarChange<ValueType> = useEvent((nextCalendarValues: ValueType) => {
...
export default function useRangeValue<ValueType extends DateType[], DateType extends object = any>(
  info: Pick<RangePickerProps<DateType>, 'generateConfig' | 'locale' | 'allowEmpty' | 'order' | 'picker'> & ReplacedPickerProps<DateType>,
  ...
  const triggerSubmit = useEvent((nextValue?: ValueType) => {
    ...
    // >>> Order
    const validateOrder =
      !order || startEmpty || endEmpty || isSame(generateConfig, locale, start, end, picker) || generateConfig.isAfter(end, start)
